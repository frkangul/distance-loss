...
from src.utils import seed_everything, setup_pl_callbacks
...
def test_pipeline(cfg: DictConfig):
    ...
    trainer = pl.Trainer(
        ...
        strategy=None
        if cfg.server.device_num == 1
        else "dp",  # 'dp' strategy is used when multiple-gpus with 1 machine.
    ...
    artifact = run.use_artifact(
        "frkangul/foreground-car-segm/model-m1l47gow:v67", type="model"
    )
    artifact_dir = artifact.download()
    trainer.test(
        model, ckpt_path=str(Path(artifact_dir, "model.ckpt"))
    )  # ckpt_path="last" to load and evaluate the last model
