    ...
    def on_validation_batch_end(
        ...
        if batch_idx == pl_module.cfg.vis.val_batch_id:
            with torch.no_grad():
                ...
                x, y, y_distance, y_distance_sum = (
                    batch["image"],
                    batch["mask"],
                    batch["distance_mask"],
                    batch["distance_mask_sum"],
                )

                # All five images and preds in one figure
                fig1, axs1 = plt.subplots(
                    pl_module.cfg.vis.img_num, 5, figsize=(50, 50)
                )  # pl_module.cfg.vis.val_dl.batch_size
                for img_id, (img_gt, mask_gt, mask_dist_gt, mask_pred) in enumerate(
                    zip(x, y, y_distance, outputs["prob_y"])
                ):  # [1, num_channels, height, width]
                    mask_pred_th = (mask_pred > 0.5).float()
                    un_img = (
                        img_gt.cpu().numpy().squeeze().transpose(1, 2, 0)
                    )  # for visualization we have to transpose back to HWC
                    img_gt = (
                        pl_module.unnorm_transform(image=un_img)["image"] * 255
                    ).to(torch.uint8)

                    axs1[img_id, 0].imshow(np.asarray(visF.to_pil_image(img_gt)))
                    axs1[img_id, 0].set_title("Image", fontsize=30)

                    axs1[img_id, 1].imshow(
                        mask_dist_gt.cpu().numpy().squeeze()
                    )  # for visualization we have to remove 3rd dimension of mask
                    axs1[img_id, 1].set_title("GT-DistanceMask", fontsize=30)

                    axs1[img_id, 2].imshow(
                        mask_gt.cpu().numpy().squeeze()
                    )  # for visualization we have to remove 3rd dimension of mask
                    axs1[img_id, 2].set_title("GT-Mask", fontsize=30)

                    axs1[img_id, 3].imshow(
                        mask_pred.cpu().numpy().squeeze()
                    )  # for visualization we have to remove 3rd dimension of mask
                    axs1[img_id, 3].set_title("Pred-Mask", fontsize=30)

                    axs1[img_id, 4].imshow(
                        mask_pred_th.cpu().numpy().squeeze()
                    )  # for visualization we have to remove 3rd dimension of mask
                ...
                img_dir = os.path.join(
                    pl_module.cfg.vis.dir,
                    f"epoch{pl_module.current_epoch}_data_pred_vis.png",
                )
                ...
                trainer.logger[1].experiment.log(
                    {
                        f"val_batch{batch_idx}_all": [
                            wandb.Image(img_dir, caption=f"val_batch{batch_idx}_all")
                        ]
                    }
                )
                ...
                for img_id, (img_gt, mask_gt, mask_pred) in enumerate(
                    zip(x, y, outputs["prob_y"])
                ):  # [1, num_channels, height, width]
                    ...
                    img = (pl_module.unnorm_transform(image=un_img)["image"] * 255).to(
                        torch.uint8
                    )

                    gt_mask_on_img = draw_segmentation_masks(
                        image=img, masks=mask_gt_th, alpha=0.7, colors=["orange"]
                    )  # Tensor[C, H, W]
                    pred_mask_on_img = draw_segmentation_masks(
                        image=img, masks=mask_pred_th, alpha=0.7, colors=["orange"]
                    )  # Tensor[C, H, W]
                    ...
                    img_dir = os.path.join(
                        pl_module.cfg.vis.dir,
                        f"epoch{pl_module.current_epoch}_val_batch{batch_idx}_image{img_id}.png",
                    )
                    fig2.savefig(img_dir, bbox_inches="tight")
                    # wandb_logger.log_image(key=f"val_batch{batch_idx}_image{img_id}", images=[img_dir])
                    trainer.logger[1].experiment.log(
                        {
                            f"val_batch{batch_idx}_image{img_id}": [
                                wandb.Image(
                                    img_dir,
                                    caption=f"val_batch{batch_idx}_image{img_id}",
                                )
                            ]
                        }
                    )
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if batch_idx == pl_module.cfg.vis.val_batch_id:
            with torch.no_grad():
                ...
                img_dir = os.path.join(
                    pl_module.cfg.vis.dir,
                    f"train_epoch{pl_module.current_epoch}_data_pred_vis.png",
                )
                fig1.savefig(img_dir, bbox_inches="tight")
                # wandb_logger.log_image(key=f"train_batch{batch_idx}_all", images=[img_dir])
                trainer.logger[1].experiment.log(
                    {
                        f"train_batch{batch_idx}_all": [
                            wandb.Image(img_dir, caption=f"train_batch{batch_idx}_all")
                        ]
                    }
                )
                ...
                for img_id, (img_gt, mask_gt, mask_pred) in enumerate(
                    ...
                    img_dir = os.path.join(
                        pl_module.cfg.vis.dir,
                        f"epoch{pl_module.current_epoch}_train_batch{batch_idx}_image{img_id}.png",
                    )
                    fig2.savefig(img_dir, bbox_inches="tight")
                    # wandb_logger.log_image(key=f"train_batch{batch_idx}_image{img_id}", images=[img_dir])
                    trainer.logger[1].experiment.log(
                        {
                            f"train_batch{batch_idx}_image{img_id}": [
                                wandb.Image(
                                    img_dir,
                                    caption=f"train_batch{batch_idx}_image{img_id}",
                                )
                            ]
                        }
                    )
    ...
    def on_validation_batch_end(
        ...
        if batch_idx == pl_module.cfg.vis.val_batch_id:
                ...
                masks = [
                    {
                        ...
                        "ground_truth": {
                            "mask_data": mask_gt.cpu().detach().numpy().squeeze(),
                            "class_labels": {1: "car"},
                        },
                    }
            ...
            wandb_logger.log_image(
                key=f"val_batch{batch_idx}_image{img_id}", images=images, masks=masks
            )
