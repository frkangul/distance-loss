...
def clear_wandb_artifacts(wandb_proj, artifact):
    ...
    api = wandb.Api(overrides={"project": wandb_proj})
...
def setup_pl_callbacks(cfg):
    ...
    model_checkpointer = ModelCheckpoint(
        ...
        filename="{epoch}-"
        + f"{cfg.exp.model}-{cfg.exp.encoder}-lr{cfg.trainer.lr}-hight{cfg.dataset.transform.image_resize_h}-width{cfg.dataset.transform.image_resize_w}-{cfg.dataset.name}",
    )
