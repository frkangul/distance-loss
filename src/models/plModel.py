...
from src.data.customDatasets import (
    DatasetFromSubset,
    CocoToSmpDataset,
    CityscapesToSmpDataset,
    random_split,
)
...
class ImageSegModel(pl.LightningModule):
    ...
    * np .cuda()
    ...
    def __init__(
        self, cfg, train_transform, val_transform, test_transform, unnorm_transform
    ):
        ...
        if cfg.exp.loss == "dist_transform":
            ...
            self.loss_focal = smp.losses.FocalLoss(
                cfg.dataset.mode
            )  # it uses focal_loss_with_logits
        self.distance_transform_metric = DistanceLoss(
            cfg.dataset.mode, from_logits=False
        )
    ...
    def _shared_step(self, batch, stage):
        ...
        with torch.no_grad():
            ...
            tp, fp, fn, tn = smp.metrics.get_stats(
                ...
                num_classes=self.cfg.dataset.output_class_num,
            )
    ...
    def _shared_step_end(self, batch_parts):
        ...
        if batch_parts["loss"].numel() != 1:  # multi-gpu case
            batch_parts["loss"] = torch.mean(
                batch_parts["loss"], dim=0
            )  # take the mean of losses from different GPUs
        return batch_parts
    ...
    def _shared_epoch_end(self, step_outputs, stage):
        with torch.no_grad():
            ...
            per_image_fbeta = smp.metrics.fbeta_score(
                tp, fp, fn, tn, reduction="micro-imagewise"
            )
            dataset_fbeta = smp.metrics.fbeta_score(tp, fp, fn, tn, reduction="micro")

            per_image_sensitivity = smp.metrics.sensitivity(
                tp, fp, fn, tn, reduction="micro-imagewise"
            )
            dataset_sensitivity = smp.metrics.sensitivity(
                tp, fp, fn, tn, reduction="micro"
            )

            per_image_specificity = smp.metrics.specificity(
                tp, fp, fn, tn, reduction="micro-imagewise"
            )
            dataset_specificity = smp.metrics.specificity(
                tp, fp, fn, tn, reduction="micro"
            )

            per_image_bIoU = smp.metrics.iou_score(
                b_tp, b_fp, b_fn, b_tn, reduction="micro-imagewise"
            )
            dataset_bIoU = smp.metrics.iou_score(
                b_tp, b_fp, b_fn, b_tn, reduction="micro"
            )
        ...
        del (
            ...
            per_image_fbeta,
            dataset_fbeta,
            per_image_sensitivity,
            dataset_sensitivity,
            per_image_specificity,
            dataset_specificity,
            per_image_bIoU,
            dataset_bIoU,
            metrics,
        ...
        if self.cfg.dataset.name == "CelebAMask-HQ":
            if stage == "fit" or stage is None:
                full_ds = CocoToSmpDataset(
                    root=os.path.join(self.cfg.dataset.dir, "train"),
            ...
            if stage == "test" or stage is None:
                self.test_ds = CocoToSmpDataset(
                    root=os.path.join(self.cfg.dataset.dir, "test"),
                    annFile=os.path.join(self.cfg.dataset.dir, "annotations_test.json"),
                    transforms=self.test_transform,
                )
        elif self.cfg.dataset.name == "Cityscapes":
            if stage == "fit" or stage is None:
                self.train_ds = CityscapesToSmpDataset(
                    self.cfg.dataset.dir,
                    split="train",
                    mode="fine",
                    transforms=self.train_transform,
                )

                self.val_ds = CityscapesToSmpDataset(
                    self.cfg.dataset.dir,
                    split="val",
                    mode="fine",
    ...
    def train_dataloader(self):
        return DataLoader(
            self.train_ds,
            batch_size=self.cfg.dataset.train_dl_batchsize,
            shuffle=True,
            num_workers=self.n_cpu - 1,
            pin_memory=True,
        )
    ...
    def val_dataloader(self):
        return DataLoader(
            self.val_ds,
            batch_size=self.cfg.dataset.val_dl_batchsize,
            shuffle=False,
            num_workers=self.n_cpu - 1,
            pin_memory=True,
        )
    ...
    def test_dataloader(self):
        if self.cfg.dataset.name == "CelebAMask-HQ":
            return DataLoader(
                self.test_ds,
                batch_size=self.cfg.dataset.test_dl_batchsize,
                shuffle=False,
                num_workers=self.n_cpu - 1,
                pin_memory=True,
            )
